...
router.post("/users", async (req, res) => {
  try {
    ...
    const validPassword = bcrypt.compare(password, user[0].password, function (err, res) {
      if (err) {
        console.warn('[Login] err ->', err);
      }
      if (res) {
        // Send JWT
        const token = jwt.sign({ username: user[0].username }, 'jwttokenkey123encrp../$$1%unique.', { expiresIn: '8h' });
